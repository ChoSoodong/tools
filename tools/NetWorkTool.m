...
+ (void)initialize{
    ...
    //打开状态栏菊花 这个方法afn会自动检测，只有网络加载缓慢，数据量较大的时候才会显示，如果需要每次都显示那就自己设置吧，系统原生代码[UIApplication sharedApplication].networkActivityIndicatorVisible = YES; yes显示no关闭。
        ...
        [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            switch (status) {
                ...
                case AFNetworkReachabilityStatusReachableViaWWAN:
                    networkStatus ? networkStatus(NetworkStatusReachableWWAN) : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWiFi:
                    networkStatus ? networkStatus(NetworkStatusReachableWiFi) : nil;
                    break;
                default:
                    break;
            }
...
#pragma mark -- 缓存描述文字
+ (NSString *)cachePolicyStr:(   CachePolicy)cachePolicy
{
    switch (cachePolicy) {
        case    CachePolicyIgnoreCache:
            return @"只从网络获取数据，且数据不会缓存在本地";
            break;
        case    CachePolicyCacheOnly:
            return @"只从缓存读数据，如果缓存没有数据，返回一个空";
            break;
        case    CachePolicyNetworkOnly:
            return @"先从网络获取数据，同时会在本地缓存数据";
            break;
        case    CachePolicyCacheElseNetwork:
            return @"先从缓存读取数据，如果没有再从网络获取";
            break;
        case    CachePolicyNetworkElseCache:
            return @"先从网络获取数据，如果没有再从缓存读取数据";
            break;
        case    CachePolicyCacheThenNetwork:
            return @"先从缓存读取数据，然后再从网络获取数据，Block将产生两次调用";
            break;
            
        default:
            return @"未知缓存策略，采用   CachePolicyIgnoreCache策略";
            break;
    }
}
...
{
    [self HTTPWithMethod:RequestMethodGET url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)POSTWithURL:(NSString *)url
        ...
        cachePolicy:(CachePolicy)cachePolicy
           callback:(HttpRequest)callback
{
    [self HTTPWithMethod:RequestMethodPOST url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)HEADWithURL:(NSString *)url
        ...
        cachePolicy:(CachePolicy)cachePolicy
           callback:(HttpRequest)callback
{
    [self HTTPWithMethod: RequestMethodHEAD url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PUTWithURL:(NSString *)url
       ...
       cachePolicy:(   CachePolicy)cachePolicy
          callback:(   HttpRequest)callback
{
    [self HTTPWithMethod:   RequestMethodPUT url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PATCHWithURL:(NSString *)url
         ...
         cachePolicy:(   CachePolicy)cachePolicy
            callback:(   HttpRequest)callback
{
    [self HTTPWithMethod:   RequestMethodPATCH url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)DELETEWithURL:(NSString *)url
          ...
          cachePolicy:(   CachePolicy)cachePolicy
             callback:(   HttpRequest)callback
{
    [self HTTPWithMethod:   RequestMethodDELETE url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
              ...
              callback:(   HttpRequest)callback
{
    ...
    if (cachePolicy == CachePolicyIgnoreCache) {
        //只从网络获取数据，且数据不会缓存在本地
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }else if (cachePolicy ==    CachePolicyCacheOnly){
    ...
    }else if (cachePolicy ==    CachePolicyNetworkOnly){
        //先从网络获取数据，同时会在本地缓存数据
    ...
    }else if (cachePolicy ==    CachePolicyCacheElseNetwork){
        //先从缓存读取数据，如果没有再从网络获取
        [self httpCacheForURL:url parameters:parameters withBlock:^(id<NSCoding> object) {
            if (object) {
                callback ? callback(object, nil) : nil;
    ...
    }else if (cachePolicy ==    CachePolicyNetworkElseCache){
    ...
    }else if (cachePolicy ==    CachePolicyCacheThenNetwork){
        ...
        //缓存策略错误，将采取    CachePolicyIgnoreCache 策略
        ATLog(@"缓存策略错误");
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }
...
#pragma mark -- 网络请求处理
+(void)httpWithMethod:(RequestMethod)method url:(NSString *)url parameters:(NSDictionary *)parameters callback:(   HttpRequest)callback{
...
+(void)dataTaskWithHTTPMethod:(   RequestMethod)method url:(NSString *)url parameters:(NSDictionary *)parameters
                      ...
                      failure:(void (^)(NSURLSessionDataTask * _Nullable, NSError * _Nonnull))failure
{
    ...
    }else if (method == RequestMethodHEAD) {
    ...
    }else if (method == RequestMethodPATCH) {
...
#pragma mark -- 自定义文件上传
+(void)uploadWithURLString:(NSString *)URLString parameters:(id)parameters constructingBodyWithBlock:(void(^)(id))formDataBlock progress:(void(^)(id))progressBlock success:(void (^)(id))successBlock failure:(void (^)(NSError *))failureBlock{
...
+ (NSString *)getMethodStr:(   RequestMethod)method{
    switch (method) {
