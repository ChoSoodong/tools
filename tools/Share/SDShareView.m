...
@interface SDShareView()<UICollectionViewDelegate,UICollectionViewDataSource>
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _itemSpace = 0;
        _itemImageSize = CGSizeMake(44, 44);
...
- (instancetype)initWithItems:(NSArray *)items countEveryRow:(NSInteger)count{
    ...
    if (self) {
        self.shareItems = items;
        self.itemSize = CGSizeMake(SCREEN_W/count, SCREEN_W/count);
        self.oneLine = NO;
...
-(void)layoutSubviews {
    ...
    //动画前置控件位置
    if (_containView) {
        _containView.frame = CGRectMake(0, SCREEN_H, SCREEN_W, height);
    }
    if (_headerView) {
        _headerView.frame = CGRectMake(0, 0, SCREEN_W, _headerView.frame.size.height);
    }
    if (_bodyView) {
        float bodyY = _headerView ? CGRectGetMaxY(_headerView.frame) : 0;
        _bodyView.frame = CGRectMake(_bodyViewEdgeInsets.left,bodyY+_bodyViewEdgeInsets.top,SCREEN_W -_bodyViewEdgeInsets.left-_bodyViewEdgeInsets.right, bodyHeight);
    ...
    if (_footerView) {
        _footerView.frame = CGRectMake(0, CGRectGetMaxY(_bodyView.frame)+_bodyViewEdgeInsets.bottom, SCREEN_W, _footerView.frame.size.height);
    }
    
    if (_cancleButton) {
        _cancleButton.frame = CGRectMake(0, height-_cancleButton.frame.size.height, SCREEN_W,_cancleButton.frame.size.height);
    }
    ...
    [UIView animateWithDuration:0.25 animations:^{
        if (weakSelf.containView) {
            weakSelf.containView.frame = CGRectMake(0, SCREEN_H - height, SCREEN_W, height);
        }
...
- (void)tappedCancel {
    [UIView animateWithDuration:0.25 animations:^{
        ...
        __weak __typeof(self) weakSelf = self;
        if (weakSelf.containView) {
            weakSelf.containView.frame = CGRectMake(0, SCREEN_H,SCREEN_W, weakSelf.containView.frame.size.height);
        }
...
- (void)showInView:(UIView *)view{
    ...
    NSInteger numberOfPerRow = SCREEN_W / _itemSize.width;
    ...
    UICollectionView *shareCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_W, height) collectionViewLayout:flowLayout];
    ...
    [shareCollectionView registerClass:[SDShareItemCell class] forCellWithReuseIdentifier:kCellIdentifier_SDShareItemCell];
    ...
    if (self.functionItems) {
        ...
        [functionCollectionView registerClass:[SDShareItemCell class] forCellWithReuseIdentifier:kCellIdentifier_SDShareItemCell];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SDShareItemCell *shareItemCell = [collectionView dequeueReusableCellWithReuseIdentifier:kCellIdentifier_SDShareItemCell forIndexPath:indexPath];
    ...
    if (collectionView == self.shareCollectionView) {
        if ([self.shareItems[indexPath.row] isKindOfClass:[NSString class]]) {
           shareItemCell.shareModel = [[SDShareModel alloc] initWithPlatformName:self.shareItems[indexPath.row]];
        }else{
           shareItemCell.shareModel = self.shareItems[indexPath.row];
        }
        
    }else{
        if ([self.functionItems[indexPath.row] isKindOfClass:[NSString class]]) {
            shareItemCell.shareModel = [[SDShareModel alloc] initWithPlatformName:self.functionItems[indexPath.row]];
        }else{
            shareItemCell.shareModel = self.functionItems[indexPath.row];
        }
    }
    shareItemCell.shareModel.shareText = _shareText;
    shareItemCell.shareModel.shareImage = _shareImage;
    shareItemCell.shareModel.shareUrl = _shareUrl;
    shareItemCell.shareModel.presentVC = _presentVC;
    
    return shareItemCell;
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    SDShareItemCell *cell = (SDShareItemCell *)[collectionView cellForItemAtIndexPath:indexPath];
    if (cell.shareModel.action) {
        cell.shareModel.action(cell.shareModel);
    }
    [self dismiss:YES];
}
